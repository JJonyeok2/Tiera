...
### 15.2 GitHub OAuth 연결 (2026-09-09)

프로덕션에 로그인 수단이 없어 아무도 평가를 남길 수 없던 상태를 해소했다.
코드 변경은 없었다 — `auth.ts`가 이미 자격증명이 있을 때만 provider를 등록하도록
짜여 있어서, Vercel에 `AUTH_GITHUB_ID` / `AUTH_GITHUB_SECRET`을 넣고 재배포한 것이 전부다.

- OAuth App 콜백: `https://tiera-sable.vercel.app/api/auth/callback/github`
- 검증: `/login`에 GitHub 버튼 노출 + `/api/auth/csrf`가 토큰 반환
  (`AUTH_SECRET`이 없으면 이 경로가 500을 낸다. 그래서 이 엔드포인트를 헬스체크로 쓴다)
- 프로덕션 더미 리뷰 2,879건 삭제 완료

### 15.3 QA에서 잡은 버그 2건 (2026-09-09)

더미 리뷰를 지운 뒤 실제 화면을 확인하다가 나왔다. 둘 다 "리뷰가 0이 된 상황"을
가정하지 않아서 생긴 문제다.

**(1) 리뷰가 사라진 모델의 `status`가 CERTIFIED로 남는다**

`recomputeCommunity()`는 리뷰가 있는 모델만 루프를 돌며 상태를 갱신했다.
리뷰가 하나도 없는 모델은 `continue`로 빠져나가서, 예전에 CERTIFIED였던 값이
그대로 남았다. 더미 2,879건을 지운 뒤 리뷰 0개짜리 모델들이 "공인" 배지를 달고 있었다.

상태는 리뷰 수에서 파생되는 값이므로 리뷰가 사라지면 같이 내려가야 한다.
갱신을 루프 밖으로 빼고 대상 전체에 한 번에 적용하는 단일 UPDATE로 바꿨다.
같은 작업에서 루프 안의 모델별 `COUNT` 쿼리(N+1)도 그룹 집계 한 번으로 접었다.

**(2) 벤치마크 탭이 표본을 "리뷰 N개"로 표기한다**

`model_score.sample_count`는 점수 종류에 따라 의미가 다르다. 커뮤니티는
"리뷰를 쓴 사람 수", 벤치마크는 "집계에 쓴 벤치마크 종류 수"다. 그런데 UI가
둘 다 "리뷰 N개"로 찍어서, 리뷰가 0건인데 벤치마크 탭에 "리뷰 5개"가 떴다.
없는 평가가 있는 것처럼 보이는, 이 사이트에서 제일 하면 안 되는 종류의 거짓말이다.

벤치마크 탭은 "벤치마크 N종"으로 표기하고, 리뷰 수에서 파생되는 표시
(공인/평가 중 배지, 점선 링)는 커뮤니티 탭에서만 렌더한다.
회귀 테스트를 `tests/e2e/ranking.spec.ts`에 추가했다.

## 16. 데이터 소스 로드맵

### 16.1 사람 평가 축 — LMArena로 전환 (예정)

커뮤니티 점수를 시드 더미로 채우면 이 사이트의 핵심인 "체감 vs 벤치마크 괴리"가
통째로 거짓말이 된다. 그래서 실제 사람 투표 데이터를 쓴다.

- 출처: `lmarena-ai/leaderboard-dataset` (HuggingFace datasets-server, 인증 불필요)
- 라이선스: **CC BY 4.0 — 출처 표기 필수** (Artificial Analysis와 동일 패턴)
- 핵심 필드: `rating`(Bradley-Terry), `vote_count`, `organization`, `category`
- `vote_count`가 베이지안 보정식의 `v`에 그대로 들어간다. 스코어링 엔진 재사용 가능.
- 카테고리 매핑: `overall`→OVERALL, `coding`→CODING, `creative_writing`→WRITING,
  `math`→REASONING, `vision` 컨피그→MULTIMODAL. 추가로 `korean` 카테고리가 있어
  "한국어 성능" 스코프를 만들 수 있다.
- 탭 라벨을 "커뮤니티 평가" → "사람 평가(LMArena)"로 바꾼다. Tiera 자체 리뷰와
  아레나 투표는 성격이 달라 합산하지 않는다. 자체 리뷰는 모델 상세 섹션으로 두고,
  쌓이면 별도 탭으로 승격한다.

**한계(반드시 표시):** LMArena 등록 개발사 27곳에 한국 회사가 없다.
LG·업스테이지·네이버·카카오 모델은 아레나 데이터가 아예 없어서 "아레나 미참여"로
표기하고 벤치마크 점수만 보여준다. `korean` 카테고리는 "한국어 프롬프트" 분류이지
"한국 모델" 분류가 아니라서 이 공백을 메우지 못한다.

### 16.2 국내 커뮤니티 수집 — 보류 (검토 완료)

한국 모델의 사람 평가 공백을 국내 커뮤니티로 메우는 안을 검토했고, 보류한다.

| 경로 | 판정 | 근거 |
|---|---|---|
| 네이버 검색/데이터랩 API | 불가 | 2026-09-07 개정 약관이 검색 결과의 저장·캐싱, AI 활용, 제3자 제공, 수익화를 금지 |
| 카카오 다음 검색 API | 불가 | 운영정책 5조 30호 — 사전 승낙 없는 복제·변경·사용 금지 |
| 디시·클리앙·OKKY·에펨코리아 | 제외 | robots.txt가 ClaudeBot·GPTBot·anthropic-ai를 명시 차단 |
| 아카라이브·루리웹 | 가능하나 보류 | AI봇 차단은 없음. 다만 노이즈 대비 신호가 티어를 정당화할 수준이 아니고 크롤러 유지보수 부담이 크다 |
| GeekNews RSS | 가능하나 보류 | 공식 Atom 피드 존재. 개발자 편향이 크고 재배포 라이선스 조항이 약관에 없다 |

법적 판단 근거: 잡코리아 v 사람인(서울고법 2016나2019365)은 **원문의 상당 부분을
복제해 재현**한 것을 DB제작자 권리 침해로 봤고, 야놀자 v 여기어때(대법 2021도1533)는
기술적 보호조치 없는 공개 API 수집을 무죄로 봤다. 즉 위험은 "긁는 행위"보다
**"원문을 저장·재현하는 것"**에 있다. 훗날 수집하더라도 원문은 저장·표시하지 않고
집계 수치만 산출하고 원본 링크로만 연결하는 설계를 전제로 한다.

**대신 채택한 방향:** 남의 커뮤니티를 긁는 대신 이 사이트가 그 커뮤니티가 된다.
GitHub OAuth를 붙여(15.2) 한국 사용자가 EXAONE·Solar를 직접 평가하게 하는 쪽이
어디서도 못 구하는 데이터이고 약관 리스크도 없다.

### 16.3 한국 모델 보조 지표 — HuggingFace 채택도 (예정)

- 출처: HuggingFace Hub API (`/api/models`), 공식·무료
- 지표: `downloads`, `likes`
...
### 17.5 점수 없는 모델은 색인하지 않는다

Artificial Analysis는 이름은 알려주지만 평가 데이터가 없는 모델을 상당수 내려준다.
누락된 항목을 0으로 채우지 않기로 했으므로(10절), 이런 모델은 `model_score` 행이
아예 생기지 않는다. 실제로 발행된 모델 557개 중 랭킹에 오르는 건 302개뿐이고,
나머지 255개의 상세 페이지는 "데이터 없음"만 찍힌 빈 문서다.

이걸 그대로 sitemap에 올렸다가 발견해서 두 군데를 막았다.

- **sitemap**: `model_score` 행이 하나라도 있는 모델만 포함한다.
- **상세 페이지**: 점수도 벤치마크 원본도 없으면 `robots: noindex, follow`.
  페이지 자체는 살려둔다 — 직접 방문이나 내부 링크는 유효해야 하고,
  나중에 데이터가 들어오면 자동으로 색인 대상이 된다.
...
"익명"에는 두 가지가 있고, 둘은 전혀 다른 물건이다.

- **표시 익명** — 로그인은 하되 목록에 이름을 감춘다
- **인증 없는 익명** — 로그인 없이 평가를 받는다

**표시 익명만 채택한다.** 인증 없는 익명은 1인 1회 제약을 강제할 방법이 없어서,
한 사람이 원하는 만큼 점수를 밀어 넣을 수 있다. 그 순간 커뮤니티 점수는 여론이 아니라
조작 결과가 되고, 이 사이트의 존재 이유인 "체감 vs 벤치마크 괴리"가 무의미해진다.
더미 리뷰를 지운 것과 같은 이유다.

### 구현

- `review.is_anonymous` 컬럼 추가 (`db/migrations/0001_*.sql`).
  작성자(`user_id`)는 그대로 저장하므로 `review_user_model_uq` 제약과 소유권 검증이
  전혀 영향을 받지 않는다.
- **익명 처리는 SQL에서 한다.**
  ...
  UI에서만 가리면 이름이 API 응답 JSON에 그대로 실려 나가서 개발자도구만 열면 보인다.
  아예 내보내지 않는 것이 유일하게 맞는 처리다.
- 폼의 체크박스는 수정 시에도 이전 선택을 유지한다(`getMyReview`가 값을 함께 반환).
- **화면에서도 독립적으로 막는다.** 표시명은 `displayAuthorName()`을 거치고,
  이 함수는 `isAnonymous`가 참이면 서버가 이름을 내려주더라도 무시한다.
  쿼리 한 겹만 두면 그 쿼리가 회귀하는 날 이름이 그대로 화면에 찍힌다.
  단위 테스트에 "서버가 이름을 내려준 경우"를 케이스로 넣어뒀다.
- E2E가 두 가지를 동시에 본다: 화면에 이름이 없을 것, **그리고 응답 본문에도 없을 것.**
  후자가 진짜 검증이다.

## 19. 배포 시 마이그레이션 (2026-09-09)

### 19.1 왜 빌드에서 도는가

운영 DB 접속 주소는 Vercel에 **Sensitive**로 저장돼 있다. Sensitive 변수는
정의상 다시 읽을 수 없다 — 대시보드에서도, `vercel env pull`로도 안 나온다.
실제로 `vercel env pull`을 돌려보니 Config로 저장한 변수 하나만 내려왔다.

즉 **로컬에서 운영 DB에 마이그레이션을 돌릴 방법이 사실상 없다.** 컬럼을 추가할
때마다 Supabase 콘솔을 열어야 하고, 잊으면 없는 컬럼을 조회하는 코드가 배포돼
런타임 500이 난다. 원인 찾기도 어렵다.

빌드 컨테이너 안에서는 그 값이 정상적으로 주입된다. 그래서 마이그레이션을
빌드 앞단에 붙였다.
...
`&&`인 것이 중요하다. 마이그레이션이 실패하면 빌드가 멈추고 배포가 나가지 않는다.
서비스는 이전 버전 그대로 살아 있다. 스키마와 코드가 어긋난 채 배포되는 것보다 낫다.

### 19.2 실행 조건 (`db/migrate-deploy.ts`)

| 상황 | 동작 |
|---|---|
| 로컬 빌드 (`VERCEL` 없음) | 건너뜀 — 빌드가 DB를 요구하면 DB 없이 빌드만 확인할 수 없다 |
| 프리뷰 배포 (`VERCEL_ENV !== production`) | 건너뜀 — 브랜치 하나 띄울 때마다 운영 스키마가 바뀌면 안 된다 |
| 프로덕션인데 주소 없음 | **실패** — 조용히 넘기면 첫 요청에서 죽는다 |
| 프로덕션 + 주소 있음 | 마이그레이션 실행 |

빌드는 재시도·롤백으로 여러 번 돌 수 있으므로 마이그레이션 SQL은 재실행에
안전하게 쓴다(`ADD COLUMN IF NOT EXISTS` 등).

### 19.3 곁다리로 잡은 것 — import 시점 예외

이 작업 중에 `db/index.ts`의 주석이 사실과 다른 것을 발견했다.
"프록시로 감싸서 import만으로는 아무 일도 안 난다"고 적혀 있었지만, 실제로는
`drizzle()`이 생성 시점에 프로퍼티를 읽고 그 접근이 풀 생성을 트리거한다.
**주소가 없으면 import에서 던진다.**

판별용 접근에 `undefined`를 돌려주는 식으로 우회해봤지만 drizzle이 그 값을
실제로 쓰기 때문에 빌드가 다른 곳에서 깨졌다. 그래서 동작은 그대로 두고
주석을 사실에 맞게 고쳤다.

대신 두 가지를 정리했다.

- 주소 해석 함수를 부작용 없는 `db/url.ts`로 분리했다. 마이그레이션 스크립트처럼
  "주소만 알면 되는" 코드가 풀 생성 경로를 끌고 들어와 raw 스택으로 죽던 문제가 사라진다.
- `app/sitemap.ts`는 `db`를 최상단이 아니라 `try` 안에서 동적 import한다.
  최상단 import는 예외가 try 밖에서 나기 때문에, DB 장애 시 정적 페이지로
  폴백하려던 17.6의 설계가 실제로는 동작하지 않았다.

## 20. 실패 처리 점검 (2026-09-09)

"DB가 죽으면 화면이 어떻게 되나"를 기준으로 전체를 훑고 빈 곳을 메웠다.

### 20.1 에러 경계가 없었다

`app/error.tsx`도 `app/global-error.tsx`도 없었다. 서버에서 예외가 나면 Next 기본
화면("Application error: a client-side exception has occurred")이 그대로 떴다.
실제로 배포 초기에 이 화면을 본 적이 있다.

- `app/error.tsx` — 헤더·푸터는 살아 있는 채로 안내와 **다시 시도** 버튼을 준다.
- `app/global-error.tsx` — 루트 레이아웃 자체가 깨졌을 때. 레이아웃을 대체하므로
  html/body를 직접 그리고, 테마 변수도 못 쓴다고 보고 색을 인라인으로 박는다.

**원문 메시지는 화면에 찍지 않는다.** 서버 예외에는 SQL이나 접속 정보가 섞여 나올
수 있다. 대신 Next가 붙여주는 `digest`를 보여준다 — 이 값으로 서버 로그에서
해당 예외를 정확히 찾을 수 있다.

DB를 실제로 내리고 확인했다. 헤더·푸터가 유지된 채 안내와 digest가 렌더된다.

### 20.2 리뷰 목록 새로고침이 조용히 실패했다

`ReviewSection.load()`가 `try/finally`만 있고 `catch`가 없었다. 정렬을 바꿨는데
요청이 실패하면 예외는 삼켜지고 목록은 그대로 남는다. 사용자는 정렬이 반영된 줄
알고 **옛 목록을 새 결과로 착각한다.** 잘못된 데이터를 보여주는 쪽이 아무것도 안
보여주는 쪽보다 나쁘다.

`res.ok`를 먼저 확인하고(500 본문은 JSON이 아닐 수 있어 바로 `json()`을 부르면
파싱 예외로 원인이 가려진다), 실패 시 안내와 재시도 버튼을 띄운다.

### 20.3 로딩 스켈레톤 — 그리고 그것이 깨뜨린 것

홈은 요청마다 DB를 읽는 동적 페이지라, 느릴 때 화면이 통째로 비어 있었다.
스켈레톤을 넣었더니 **"없는 모델은 404" 테스트가 200을 받으면서 깨졌다.**

`loading.tsx`는 해당 세그먼트와 **그 하위 전체**를 스트리밍으로 바꾼다.
스트리밍은 본문보다 헤더가 먼저 나가므로, 뒤늦게 호출된 `notFound()`가 상태
코드를 404로 바꾸지 못한다. 없는 페이지가 200을 주면 검색엔진에는 soft 404로
잡혀 17장에서 맞춰둔 색인 설정까지 망가진다.

`(home)` 라우트 그룹으로 옮겨 적용 범위를 홈으로만 좁혔다. 라우트 그룹은 URL에
영향을 주지 않는다. 회귀 테스트를 `tests/e2e/seo.spec.ts`에 넣었다.

### 20.4 이미 되어 있던 것

- `/compare` — 슬러그가 없거나 1개면 안내 화면 (`models.length < 2`)
- `/models/[slug]` — 없는 모델은 `notFound()`
- 랭킹 무한 스크롤 — 실패 시 재시도 버튼
- 크론 3종 — `CRON_SECRET` 미설정 503 / 불일치 401, 수집 실패 시 아무것도 지우지 않음
- `app/sitemap.ts` — DB 실패 시 정적 페이지만 200으로 (19.3에서 동적 import로 고쳐 실제 동작하게 됨)
- 테마 초기화 — localStorage 접근을 try/catch로 감쌈

### 20.5 남은 것

- **레이트 리미터가 인메모리다.** 서버리스에서는 인스턴스마다 카운터가 따로 생겨
  사실상 제한이 없다. 실트래픽 전에 Redis로 교체해야 한다 (`lib/rate-limit.ts`).

## 21. 첫 화면을 벤치마크로 (2026-09-09)

첫 실사용 평가가 들어온 직후 홈이 이렇게 떴다.

```
커뮤니티 평가 · 종합 기준 · 2개 모델
```

벤치마크 탭에는 302개가 있는데, 처음 들어온 사람은 2개짜리 랭킹을 본다.
리다이렉트 조건을 "커뮤니티 점수가 **하나라도** 있으면 커뮤니티 탭"으로 잡은 탓이다.
리뷰가 0개인 경우만 생각했고 2개인 경우를 생각하지 않았다.

**기본 점수 종류를 벤치마크로 바꿨다.** 첫 화면은 항상 채워져 있어야 한다.

덕분에 `hasCommunityScores()` 조회와 307 리다이렉트가 통째로 필요 없어졌다.
`/`가 곧 벤치마크 랭킹이므로 sitemap의 `/?type=BENCHMARK` 항목도 중복이라 뺐다.

### 21.1 기본값은 한 곳에만 둔다

바꾸면서 같은 기본값이 **세 군데**에 흩어져 있는 것을 발견했다 —
`lib/params.ts`(서버), `Header`(탭 강조), `RankingList`(표본 라벨).
서버만 벤치마크로 바꿨더니 서버는 벤치마크 데이터를 그리는데 탭은 커뮤니티가
켜지고 행에는 "공인 · 리뷰 7개"가 붙는, 화면 안에서 앞뒤가 안 맞는 상태가 됐다.

`DEFAULT_SCORE_TYPE`을 `lib/params.ts`에 두고 세 곳 모두 `parseScoreType()`을
쓰게 했다. 클라이언트 컴포넌트도 이 모듈을 쓸 수 있다 — 타입만 import하므로
번들에 DB 코드가 딸려오지 않는다.

## 22. 점수 종류 전환을 화면 중앙으로 (2026-09-09)

전환 토글이 헤더 우측에, 테마 버튼·로그인과 나란히 있었다. 두 순위를 나란히 놓고
그 차이를 보는 게 이 사이트의 전부인데, **그 축이 유틸리티 버튼처럼 보였다.**
구석의 작은 토글은 아무도 누르지 않는다.

### 22.1 목록 위 두 칸 스위치

`components/ranking/ScoreTypeSwitch.tsx`를 만들어 필터 위로 올렸다.
각 칸에 **모델 수와 한 줄 설명**을 붙인다.

```
┌──────────────────┬──────────────────┐
│ 벤치마크    302개 │ 커뮤니티 평가  2개 │
│ 공개 시험 성적    │ 써 본 사람들의 체감 │
└──────────────────┴──────────────────┘
```

"지금 무슨 순위를 보고 있는지"와 "반대쪽에는 뭐가 있는지"가 동시에 보인다.
개수는 `getScoreTypeTotals()`가 **현재 필터를 그대로 적용해서** 센다 —
"한국 모델만" 상태인데 전체 개수가 떠 있으면 눌렀을 때 나오는 결과와 어긋난다.
목록 조회와 서로 의존하지 않으므로 `Promise.all`로 함께 던진다.

스위치가 개수를 표시하게 되면서 그 아래 있던
`벤치마크 · 종합 기준 · 19개 모델` 줄은 같은 말의 반복이 되어 지웠다.
점수 종류는 스위치가, 카테고리는 탭의 선택 상태가 이미 말해준다.

### 22.2 테마 토글을 아이콘 하나로

라이트·다크·시스템 세 버튼이 항상 펼쳐져 헤더 폭을 차지하고 있었다.
하루에 한 번 쓸까 말까 한 설정이다. 아이콘 하나 + 드롭다운으로 접었다.

옵션 버튼은 접힌 상태에서도 `role="radio"`와 `title`을 유지한다 —
접근성 트리와 E2E 선택자가 둘 다 그 속성에 걸려 있다.
E2E는 "펼치고 고르는" 헬퍼로 감쌌다.

### 22.3 로고 옆 설명 문구 제거

전환을 옮기면서 로고 옆 문구를 `AI 모델 커뮤니티 평가`에서
`벤치마크 vs 체감 평가`로 고쳤다. 기본 탭이 벤치마크가 됐으니 문구도 맞춰야
한다고 봤는데, **바로 아래 스위치가 두 축을 이름과 개수까지 말하고 있어서
같은 문장이 화면에 두 번 나왔다.** 실사용 화면을 보고서야 드러났다.

문구를 아예 뺐다. 헤더는 **로고 / 검색 / 유틸(테마·계정)** 세 덩어리가 되고,
검색 입력이 빈 폭을 가져간다. 사이트 성격은 `<title>`과 OG 이미지가 이미
"벤치마크 vs 체감 평가"로 말하고 있으므로 화면에서 또 말할 필요가 없다.
